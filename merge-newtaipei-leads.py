#!/usr/bin/env python3
"""Merge New Taipei Scout corpus into restaurants.json. Merchant channels only; dedupe name+district & order URL."""
...
CORPUS = Path("/workspace/newtaipei-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "nt-scout-lead"

# Import deliveryMin infer from sibling backfill script
...
NT_DISTRICT_KEYS = [
    "板橋", "中和", "永和", "新莊", "三重", "蘆洲", "土城", "樹林", "淡水", "林口",
    "汐止", "新店", "八里", "深坑", "三峽", "五股", "鶯歌", "瑞芳", "金山", "三芝",
    "石門", "坪林", "貢寮", "萬里", "雙溪", "平溪", "烏來", "石碇", "泰山",
]
...
PHONE_RE = re.compile(
    ...
    r"|\(?0?2\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Taipei landline w/ optional (02)
    r"|\d{4}[-\s]?\d{4}"  # bare 8-digit local like 8969-7199 / 82214309
    r")",
)
...
DELIVERY_EVIDENCE_RE = re.compile(
    ...
    r"(?:\d+|[一二三四五六七八九十兩]+)\s*個以上|"  # quantity threshold without explicit 送
...
def pick_district(area: str) -> str:
    area_n = nfkc(area)
    if re.search(r"多店|多門市|多區|multi|citywide", area_n, re.I):
        return "新北多區"
    found = []
    for key in NT_DISTRICT_KEYS:
        if key in area_n:
            found.append(key)
    # Prefer longer / more specific when nested names collide — none do here
    if len(found) == 1:
        return DISTRICT_CANON[found[0]]
    if len(found) > 1:
        return "新北多區"
    if area_n.strip() in {"新北市", "新北", "臺北縣"}:
        return "新北多區"
...
def has_delivery_evidence(lead: dict) -> bool:
    ...
    if not DELIVERY_EVIDENCE_RE.search(blob):
        return False
    # Private kitchen without realtime order channel
    if PRIVATE_KITCHEN_RE.search(blob):
        ...
        if not realtime:
            return False
    # Empty Oddle shell: Oddle URL but no enableDelivery/eligible true and no other delivery wording beyond host
    ou = lead.get("orderUrl") or ""
    if "oddle.me" in ou.lower():
        if re.search(r"enableDelivery\s*:\s*false|eligibleForDelivery\s*:\s*false|空殼|empty", blob, re.I):
            return False
    return True
...
def parse_leads(md: str) -> list[dict]:
    ...
    for m in LEAD_RE.finditer(md):
        ...
        if len(parts) == 9:
            ...
            continue
        # Only New Taipei
        if city and "台北" in city and "新北" not in city:
            continue
        ...
        # dinbendon shop=NNNN without full URL
        for mshop in re.finditer(r"dinbendon\s+shop\s*=\s*(\d+)", contact + " " + notes, re.I):
            u = f"https://www.dinbendon.net/do/shop/{mshop.group(1)}"
        ...
        if not phone:
            ...
            if ph2:
                ...
                if phone and not phone.startswith("0") and len(re.sub(r"\D", "", phone)) == 8:
                    phone = "02-" + phone[:4] + "-" + phone[4:] if "-" not in phone else "02-" + phone
        else:
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"nt-scout-corpus-{num}", f"nt-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"nt-lead-{num}-{TODAY.replace('-', '')}-x"
...
def lead_to_record(lead: dict, rid: str) -> dict:
    ...
    # Parse deliveryMin from evidence (+ notes as terms/range stand-ins)
    probe = {
...
def primary_is_platform(lead: dict) -> bool:
    """Skip if only UE/FP URLs and no merchant channel."""
    ...
    if non_platform:
        return False
    # only platform/directory/booking URLs and no phone/LINE @
...
def merge_into(root: Path, md: str) -> dict:
    ...
    for lead in leads:
        ...
        if nn in names_any:
            ...
            if dist == "新北多區" or "新北多區" in existing_dists or "台北多區" in existing_dists:
                note_skip(lead, "dup_name_district")
                continue
            ...
            for ed in existing_dists:
                ...
                if short and short in area:
                    overlap = True
                    break
                # also Taipei district short names already present
                tshort = ed.replace("區", "")
                if tshort and tshort in area and ed.endswith("區"):
                    # only if same city-ish — skip if area mentions that Taipei district
                    pass
            if overlap:
                note_skip(lead, "dup_name_district")
                continue
    ...
    report = {
        ...
        "sample_new_names": [r["name"] for r in added[:5]],
        "added_ids": [r["id"] for r in added],
        "added_names": [r["name"] for r in added],
        "skipped_sample": skipped_sample,
        "policy": (
            "KEEP New Taipei merchant channels only (own web / Oddle / LINE / phone / iCHEF / "
            "dudoo / Ocard / dinbendon / 268web / imenu / inline.app/order). Require delivery "
            "evidence (會送/外送/enableDelivery/運費門檻等). Exclude UE/FP-primary, booking-only, "
            "empty Oddle shells, private kitchen without realtime order. Dedupe name+district "
            "(incl. 新北多區) and channel URLs/brand hosts. IDs: nt-scout-lead-{n}."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "newtaipei-leads-merge-report.json"
            ...
            json.dumps(
                {
                    ...
                    **{k: report[k] for k in (
                        ...
                        "skip_reasons", "sample_new_names", "added_with_deliveryMinLabel",
                    )},
                },
                ensure_ascii=False,
                indent=2,
