#!/usr/bin/env python3
"""Merge Yunlin Scout corpus into restaurants.json.
All leads city=雲林縣 (county only); town goes in district.
...
CORPUS = Path("/workspace/yunlin-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "yl-scout-lead"
...
# 雲林縣 towns (市／鎮／鄉) — alias short → canonical
YL_DISTRICT_CANON = {
    "斗六": "斗六市", "斗六市": "斗六市",
    "斗南": "斗南鎮", "斗南鎮": "斗南鎮",
    "虎尾": "虎尾鎮", "虎尾鎮": "虎尾鎮",
    ...
    "東勢": "東勢鄉", "東勢鄉": "東勢鄉",
    "褒忠": "褒忠鄉", "褒忠鄉": "褒忠鄉",
    "台西": "台西鄉", "臺西": "台西鄉", "台西鄉": "台西鄉", "臺西鄉": "台西鄉",
    "元長": "元長鄉", "元長鄉": "元長鄉",
    "四湖": "四湖鄉", "四湖鄉": "四湖鄉",
    "口湖": "口湖鄉", "口湖鄉": "口湖鄉",
    "水林": "水林鄉", "水林鄉": "水林鄉",
    "雲林縣多區": "雲林縣多區", "雲林多區": "雲林縣多區",
}
DISTRICT_CANON = dict(YL_DISTRICT_CANON)
DISTRICT_SHORT = {
    "斗六市": "斗六", "斗南鎮": "斗南", "虎尾鎮": "虎尾", "西螺鎮": "西螺",
    "土庫鎮": "土庫", "北港鎮": "北港", "古坑鄉": "古坑", "大埤鄉": "大埤",
    "莿桐鄉": "莿桐", "林內鄉": "林內", "二崙鄉": "二崙", "崙背鄉": "崙背",
    "麥寮鄉": "麥寮", "東勢鄉": "東勢", "褒忠鄉": "褒忠", "台西鄉": "台西",
    "元長鄉": "元長", "四湖鄉": "四湖", "口湖鄉": "口湖", "水林鄉": "水林",
}
...
ADDR_RE = re.compile(
    r"((?:雲林縣)?[\u4e00-\u9fff]{0,8}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
def pick_district(area: str, city: str) -> str:
    ...
    for tok in re.split(r"[・·/／、,，\s（）()→\-]+", area_n):
        ...
        if not tok:
            continue
        if tok in YL_DISTRICT_CANON:
            return YL_DISTRICT_CANON[tok]
    if area_n.strip() in {city, "雲林", "雲林縣"}:
        return multi_fallback
    return multi_fallback
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"yl-scout-corpus-{num}", f"yl-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"yl-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    report = {
        ...
        "policy": (
            ...
            "hosts. city=雲林縣 only (town in district); IDs: yl-scout-lead-{n}. Phones include 05-."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "yunlin-leads-merge-report.json"
