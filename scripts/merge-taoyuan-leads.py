#!/usr/bin/env python3
"""Merge Taoyuan Scout corpus into restaurants.json. Merchant channels only; dedupe name+district & order URL."""
...
CORPUS = Path("/workspace/taoyuan-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "ty-scout-lead"
...
# Taoyuan districts (蘆竹 vs Kaohsiung 路竹 — name differs; SEO slug handled separately)
DISTRICT_CANON = {
    "桃園": "桃園區", "桃園區": "桃園區",
    "中壢": "中壢區", "中壢區": "中壢區",
    "平鎮": "平鎮區", "平鎮區": "平鎮區",
...
DISTRICT_SHORT = {
    ...
    if not k.endswith("區") and k != "桃園多區"
}

TY_DISTRICT_KEYS = [
    "桃園", "中壢", "平鎮", "八德", "楊梅", "蘆竹", "大溪", "龍潭",
    "龜山", "大園", "觀音", "新屋", "復興",
]
...
PHONE_RE = re.compile(
    ...
    r"|\(?0?3\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Taoyuan / Hsinchu landline
...
ADDR_RE = re.compile(
    r"((?:桃園市)?[\u4e00-\u9fff]{0,6}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
def pick_district(area: str) -> str:
    area_n = nfkc(area)
    if re.search(r"多店|多門市|多區|multi|citywide|桃園市區|會議餐盒", area_n, re.I):
        return "桃園多區"
    found = []
    for key in TY_DISTRICT_KEYS:
        if key in area_n:
            found.append(key)
    if len(found) == 1:
        return DISTRICT_CANON[found[0]]
    if len(found) > 1:
        # Prefer longer / more specific (e.g. avoid matching 桃園 city prefix over district)
        # If "桃園" and another district both match (e.g. "桃園市中壢區"), drop bare 桃園
        if "桃園" in found and len(found) > 1:
            found = [k for k in found if k != "桃園"]
        found.sort(key=len, reverse=True)
        return DISTRICT_CANON[found[0]]
    if area_n.strip() in {"桃園市", "桃園"}:
        return "桃園多區"
...
def parse_leads(md: str) -> list[dict]:
    ...
    for m in LEAD_RE.finditer(md):
        ...
        if len(parts) == 9:
            ...
            continue
        if city and ("台北" in city or "新北" in city or "台中" in city or "高雄" in city) and "桃園" not in city:
            continue
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"ty-scout-corpus-{num}", f"ty-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"ty-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    for lead in leads:
        ...
        if nn in names_any:
            ...
            multi_markers = {"桃園多區", "台中多區", "新北多區", "台北多區", "高雄多區"}
    ...
    report = {
        ...
        "policy": (
            "KEEP Taipei+New Taipei+Taichung+Kaohsiung. Merchant channels only (own web / Oddle / LINE / phone / "
            ...
            "hosts. city=桃園市; districts with 區. IDs: ty-scout-lead-{n}."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "taoyuan-leads-merge-report.json"
