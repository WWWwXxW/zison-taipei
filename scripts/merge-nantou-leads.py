#!/usr/bin/env python3
"""Merge Nantou Scout corpus into restaurants.json.
All leads city=南投縣 (county only); town goes in district.
...
CORPUS = Path("/workspace/nantou-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "nto-scout-lead"
...
# 南投縣 towns (市／鎮／鄉) — alias short → canonical
NT_DISTRICT_CANON = {
    "南投市": "南投市", "南投": "南投市",
    "埔里": "埔里鎮", "埔里鎮": "埔里鎮",
    "草屯": "草屯鎮", "草屯鎮": "草屯鎮",
    ...
    "信義": "信義鄉", "信義鄉": "信義鄉",
    "仁愛": "仁愛鄉", "仁愛鄉": "仁愛鄉",
    "南投縣多區": "南投縣多區", "南投多區": "南投縣多區",
}
DISTRICT_CANON = dict(NT_DISTRICT_CANON)
DISTRICT_SHORT = {
    "南投市": "南投市",  # keep full — do not strip 市 for overlap checks ambiguously
    "埔里鎮": "埔里", "草屯鎮": "草屯", "竹山鎮": "竹山", "集集鎮": "集集",
    "名間鄉": "名間", "鹿谷鄉": "鹿谷", "中寮鄉": "中寮", "魚池鄉": "魚池",
    "國姓鄉": "國姓", "水里鄉": "水里", "信義鄉": "信義", "仁愛鄉": "仁愛",
}

CITY_DISTRICT_KEYS = {
    "南投縣": [
        "南投市", "南投", "埔里", "草屯", "竹山", "集集",
        "名間", "鹿谷", "中寮", "魚池", "國姓", "水里", "信義", "仁愛",
    ],
}
...
PHONE_RE = re.compile(
    ...
    r"|0?49[-\s]?\d{3}[-\s]?\d{4}"  # Nantou 049-xxx-xxxx (before generic 04)
...
# Also capture "LINE ID: foo" / "Line 點餐 ID: foo" without @
LINE_ID_RE = re.compile(r"(?:LINE|Line)\s*(?:點餐\s*)?ID\s*[:：]?\s*@?([a-zA-Z0-9._-]{3,})", re.I)
ADDR_RE = re.compile(
    r"((?:南投縣)?[\u4e00-\u9fff]{0,8}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
def pick_district(area: str, city: str) -> str:
    ...
    for tok in re.split(r"[・·/／、,，\s（）()→\-]+", area_n):
        ...
        if not tok:
            continue
        if tok in NT_DISTRICT_CANON:
            return NT_DISTRICT_CANON[tok]
    if area_n.strip() in {city, "南投", "南投縣"}:
        return multi_fallback
    return multi_fallback
...
def channel_urls_and_phone(contact: str) -> tuple[list[str], list[str], str | None]:
    ...
    # Skip literal UNKNOWN tokens that might confuse bare digits
...
def format_phone(phone: str | None) -> str | None:
    ...
    if len(digits) == 9 and digits.startswith("49"):
        ...
        return f"{digits[:3]}-{digits[3:6]}-{digits[6:]}"
    if len(digits) == 8:
        return f"049-{digits[:3]}-{digits[3:]}"
...
def make_id(num: int, existing_ids: set[str]) -> str:
    # Note: nt-scout-lead* is New Taipei; Nantou uses nto-scout-lead*
    for cand in (f"{ID_PREFIX}-{num}", f"nto-scout-corpus-{num}", f"nto-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"nto-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    report = {
        ...
        "policy": (
            ...
            "hosts. city=南投縣 only (town in district); IDs: nto-scout-lead-{n} (nt-scout-lead reserved for New Taipei). Phones include 049-."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "nantou-leads-merge-report.json"
