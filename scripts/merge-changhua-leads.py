#!/usr/bin/env python3
"""Merge Changhua Scout corpus into restaurants.json.
Preserves city=彰化市 and city=彰化縣 as separate values.
...
CORPUS = Path("/workspace/changhua-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "ch-scout-lead"
...
# 彰化市 — no 區 subdivision; use 「彰化市」 as district (single-level city)
CH_CITY_DISTRICT_CANON = {
    "彰化市": "彰化市",
    "彰化": "彰化市",
    "彰化市多區": "彰化市多區",
}
# 彰化縣 districts (市／鎮／鄉)
CH_COUNTY_DISTRICT_CANON = {
    "員林": "員林市", "員林市": "員林市",
    "鹿港": "鹿港鎮", "鹿港鎮": "鹿港鎮",
    "和美": "和美鎮", "和美鎮": "和美鎮",
    "北斗": "北斗鎮", "北斗鎮": "北斗鎮",
...
DISTRICT_CANON = {**CH_CITY_DISTRICT_CANON, **CH_COUNTY_DISTRICT_CANON}
DISTRICT_SHORT = {
    "彰化市": "彰化",
    "員林市": "員林", "鹿港鎮": "鹿港", "和美鎮": "和美", "北斗鎮": "北斗",
    "溪湖鎮": "溪湖", "田中鎮": "田中", "二林鎮": "二林",
    "線西鄉": "線西", "伸港鄉": "伸港", "福興鄉": "福興", "秀水鄉": "秀水",
    "花壇鄉": "花壇", "芬園鄉": "芬園", "大村鄉": "大村", "埔鹽鄉": "埔鹽",
    "埔心鄉": "埔心", "永靖鄉": "永靖", "社頭鄉": "社頭",
}

CITY_DISTRICT_KEYS = {
    "彰化市": ["彰化市", "彰化"],
    "彰化縣": [
        "員林", "鹿港", "和美", "北斗", "溪湖", "田中", "二林",
        "線西", "伸港", "福興", "秀水", "花壇", "芬園", "大村",
        "埔鹽", "埔心", "永靖", "社頭",
    ],
}
...
PHONE_RE = re.compile(
    ...
    r"|\(?0?4\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Changhua / Taichung landline
...
ADDR_RE = re.compile(
    r"((?:彰化[市縣])?[\u4e00-\u9fff]{0,8}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
def pick_district(area: str, city: str) -> str:
    area_n = nfkc(area)
    multi_fallback = f"{city}多區" if city in ("彰化市", "彰化縣") else "彰化市多區"
    ...
    if found:
        ...
        # For 彰化市 city: area often 「彰化市（…）」 — keep district 「彰化市」
        # not multi, unless explicitly multi
        if city == "彰化市" and found[0] in ("彰化市", "彰化"):
            return "彰化市"
    ...
    for tok in re.split(r"[・·/／、,，\s（）()→\-]+", area_n):
        ...
        if not tok:
            continue
        if city == "彰化市" and tok in CH_CITY_DISTRICT_CANON:
            return CH_CITY_DISTRICT_CANON[tok]
        if city == "彰化縣" and tok in CH_COUNTY_DISTRICT_CANON:
            return CH_COUNTY_DISTRICT_CANON[tok]
    if area_n.strip() in {city, "彰化"}:
        return "彰化市" if city == "彰化市" else multi_fallback
    # County with only street / unknown town → multi
    if city == "彰化市":
        return "彰化市"
    return multi_fallback
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"ch-scout-corpus-{num}", f"ch-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"ch-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    report = {
        ...
        "policy": (
            ...
            "hosts. city=彰化市 OR 彰化縣 (separate); IDs: ch-scout-lead-{n}. Phones include 04-."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "changhua-leads-merge-report.json"
