#!/usr/bin/env python3
"""Merge Miaoli Scout corpus into restaurants.json.
All leads city=苗栗縣 (county only); town goes in district.
Merchant channels only; dedupe name+district & order URL.
Cull #14 青畑九號豆製所（已歇業） at merge."""
...
CORPUS = Path("/workspace/miaoli-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "ml-scout-lead"
FORCE_CULLED_NUMS = {14}  # 青畑九號豆製所（頭份建國店）— confirmed closed / 歇業
...
# 苗栗縣 towns (市／鎮／鄉) — alias short → canonical
ML_DISTRICT_CANON = {
    "苗栗市": "苗栗市", "苗栗": "苗栗市",
    "頭份市": "頭份市", "頭份": "頭份市", "頭份鎮": "頭份市",
    "竹南鎮": "竹南鎮", "竹南": "竹南鎮",
    "後龍鎮": "後龍鎮", "後龍": "後龍鎮",
    "通霄鎮": "通霄鎮", "通霄": "通霄鎮",
    "苑裡鎮": "苑裡鎮", "苑裡": "苑裡鎮",
    ...
    "泰安鄉": "泰安鄉", "泰安": "泰安鄉",
    "三灣鄉": "三灣鄉", "三灣": "三灣鄉",
    "苗栗縣多區": "苗栗縣多區", "苗栗多區": "苗栗縣多區",
}
DISTRICT_CANON = dict(ML_DISTRICT_CANON)
DISTRICT_SHORT = {
    "苗栗市": "苗栗",
    "頭份市": "頭份", "竹南鎮": "竹南", "後龍鎮": "後龍", "通霄鎮": "通霄",
    "苑裡鎮": "苑裡", "卓蘭鎮": "卓蘭",
    "造橋鄉": "造橋", "西湖鄉": "西湖", "頭屋鄉": "頭屋", "公館鄉": "公館",
    "銅鑼鄉": "銅鑼", "三義鄉": "三義", "南庄鄉": "南庄", "獅潭鄉": "獅潭",
    "泰安鄉": "泰安", "三灣鄉": "三灣",
}

CITY_DISTRICT_KEYS = {
    "苗栗縣": [
        "苗栗市", "苗栗", "頭份市", "頭份", "竹南鎮", "竹南",
        "後龍鎮", "後龍", "通霄鎮", "通霄", "苑裡鎮", "苑裡",
        "卓蘭鎮", "卓蘭", "造橋鄉", "造橋", "西湖鄉", "西湖",
        "頭屋鄉", "頭屋", "公館鄉", "公館", "銅鑼鄉", "銅鑼",
        "三義鄉", "三義", "南庄鄉", "南庄", "獅潭鄉", "獅潭",
        "泰安鄉", "泰安", "三灣鄉", "三灣",
    ],
}
...
PHONE_RE = re.compile(
    ...
    r"|\(?0?37\)?[-\s]?\d{3}[-\s]?\d{3,4}"  # Miaoli 037- (before generic 03)
    r"|0?49[-\s]?\d{3}[-\s]?\d{4}"  # Nantou 049
    r"|\(?0?5\)?[-\s]?\d{3,4}[-\s]?\d{3,4}"  # Yunlin / Chiayi landline
    r"|\(?0?4\)?[-\s]?\d{3,4}[-\s]?\d{3,4}"  # Changhua / Taichung
...
LINE_ID_RE = re.compile(r"(?:LINE|Line)\s*(?:點餐\s*)?ID\s*[:：]?\s*@?([a-zA-Z0-9._-]{3,})", re.I)
ADDR_RE = re.compile(
    r"((?:苗栗縣)?[\u4e00-\u9fff]{0,8}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
def pick_district(area: str, city: str) -> str:
    ...
    for tok in re.split(r"[・·/／、,，\s（）()→\-]+", area_n):
        ...
        if not tok:
            continue
        if tok in ML_DISTRICT_CANON:
            return ML_DISTRICT_CANON[tok]
    if area_n.strip() in {city, "苗栗", "苗栗縣"}:
        return multi_fallback
    return multi_fallback
...
def channel_urls_and_phone(contact: str) -> tuple[list[str], list[str], str | None]:
    ...
    for m in LINE_ID_RE.finditer(contact):
        lid = m.group(1)
        candidate = f"https://line.me/R/ti/p/@{lid}"
        if not any(lid.lower() in x.lower() for x in line_urls):
            line_urls.append(candidate)
...
def format_phone(phone: str | None) -> str | None:
    ...
    # Miaoli landline 037-xxxxxx (9 digits) or 037-xxx-xxxx (10)
    if len(digits) == 9 and digits.startswith("037"):
        return f"{digits[:3]}-{digits[3:]}"
    if len(digits) == 10 and digits.startswith("037"):
        return f"{digits[:3]}-{digits[3:6]}-{digits[6:]}"
    if len(digits) == 8 and not digits.startswith("0"):
        # bare local — prefer 037 for this corpus
        return f"037-{digits}"
    # Nantou landline 049-xxx-xxxx (before generic 04)
    if len(digits) == 10 and digits.startswith("049"):
        return f"{digits[:3]}-{digits[3:6]}-{digits[6:]}"
    if len(digits) == 9 and digits.startswith("49"):
        digits = "0" + digits
        return f"{digits[:3]}-{digits[3:6]}-{digits[6:]}"
...
def has_delivery_evidence(lead: dict) -> bool:
    ...
    if not DELIVERY_EVIDENCE_RE.search(blob):
        return False
    if PRIVATE_KITCHEN_RE.search(blob) or PRIVATE_KITCHEN_RE.search(lead.get("name") or ""):
...
def _parse_one(num: int, name: str, rest: str, batch_of: dict[int, int], force_culled: bool = False) -> dict | None:
    ...
    if (
        force_culled
        or num in FORCE_CULLED_NUMS
        or "culled" in conf_l
        or "culled" in name.lower()
        or "歇業" in confidence
    ):
        confidence = "culled"
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"ml-scout-corpus-{num}", f"ml-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"ml-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    report = {
        ...
        "policy": (
            ...
            "iCHEF / dudoo / Ocard / dinbendon / 268web / imenu / iding / inline.app/order / SKM / nidin). Require "
            ...
            "hosts. city=苗栗縣 only (town in district); IDs: ml-scout-lead-{n}; cull #14 青畑. Phones include 037-."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "miaoli-leads-merge-report.json"
            ...
            json.dumps(
                {
                    ...
                    **{k: report[k] for k in (
                    ...
                    "skipped_sample": report["skipped_sample"],
