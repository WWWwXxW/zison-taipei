#!/usr/bin/env python3
"""Merge Tainan Scout corpus into restaurants.json. Merchant channels only; dedupe name+district & order URL."""
...
CORPUS = Path("/workspace/tainan-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "tn-scout-lead"
...
DISTRICT_SHORT = {
    v: k for k, v in DISTRICT_CANON.items()
    if not k.endswith("區") and k not in {"台南多區", "跨區品牌", "跨區"}
}

# Longer / more specific keys first so substring scan prefers 東山 over 東, 安南 over 南, etc.
TN_DISTRICT_KEYS = [
    "中西", "安平", "安南", "永康", "歸仁", "新化", "左鎮", "玉井", "楠西", "南化",
    "仁德", "關廟", "龍崎", "官田", "麻豆", "佳里", "西港", "七股", "將軍", "學甲",
    "北門", "新營", "後壁", "白河", "東山", "六甲", "下營", "柳營", "鹽水", "善化",
    "大內", "山上", "新市", "安定",
    "東", "南", "北",
]
...
PHONE_RE = re.compile(
    ...
    r"|\(?0?6\)?[-\s]?\d{3,4}[-\s]?\d{3,4}"  # Tainan landline (7- or 8-digit local)
    r"|\(?0?7\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Kaohsiung
    r"|\(?0?4\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Taichung
    r"|\(?0?3\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Taoyuan / Hsinchu
...
ADDR_RE = re.compile(
    r"((?:台南市)?[\u4e00-\u9fff]{0,6}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
DELIVERY_EVIDENCE_RE = re.compile(
    r"(外送|會送|會自送|可以送|可送|才送|就送|就可以送|親送|配送|宅配|自營外送|電話外送|enableDelivery|"
...
def pick_district(area: str) -> str:
    area_n = nfkc(area)
    if re.search(r"多店|多門市|多區|multi|citywide|跨區|台南市區|會議餐盒", area_n, re.I):
        return "台南多區"
    found = []
    for key in TN_DISTRICT_KEYS:
        if key in area_n:
            found.append(key)
    if len(found) == 1:
        return DISTRICT_CANON[found[0]]
    if len(found) > 1:
        # Prefer longer / more specific (東山 over 東, 安南 over 南, 北門 over 北)
        found.sort(key=len, reverse=True)
        return DISTRICT_CANON[found[0]]
    if area_n.strip() in {"台南市", "台南"}:
        return "台南多區"
...
def format_phone(phone: str | None) -> str | None:
    ...
    if len(digits) == 9 and digits.startswith("06"):
        # 06 + 7-digit local (e.g. 06-2233-690)
        return f"{digits[:2]}-{digits[2:6]}-{digits[6:]}"
...
def parse_leads(md: str) -> list[dict]:
    ...
    for m in LEAD_RE.finditer(md):
        ...
        if len(parts) == 9:
            ...
            continue
        if city and ("台北" in city or "新北" in city or "台中" in city or "高雄" in city or "桃園" in city) and "台南" not in city:
            continue
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"tn-scout-corpus-{num}", f"tn-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"tn-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    _MULTI = (
        ...
        "order.place", "o.easystore.co", "web66.tw",
    )
    ...
    def _brand_host(u: str | None) -> str | None:
        ...
        if any(h == s or h.endswith("." + s) for s in _MULTI):
            return None
        # Oddle brand subdomains and web66 shop hosts are per-brand — keep them
        return h
    ...
    for lead in leads:
        ...
        if nn in names_any:
            ...
            multi_markers = {"桃園多區", "台中多區", "新北多區", "台北多區", "高雄多區", "台南多區"}
    ...
    report = {
        ...
        "policy": (
            "KEEP Taipei+New Taipei+Taichung+Kaohsiung+Taoyuan. Merchant channels only (own web / Oddle / LINE / phone / "
            "iCHEF / dudoo / Ocard / dinbendon / 268web / imenu / iding / inline.app/order). Require "
            ...
            "hosts. city=台南市; districts with 區. IDs: tn-scout-lead-{n}."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "tainan-leads-merge-report.json"
