#!/usr/bin/env python3
"""Merge Hsinchu Scout corpus into restaurants.json.
Preserves city=新竹市 and city=新竹縣 as separate values.
...
CORPUS = Path("/workspace/hsinchu-own-delivery-leads.md")
TODAY = date.today().isoformat()
ID_PREFIX = "hc-scout-lead"
...
DISTRICT_CANON = {**HC_CITY_DISTRICT_CANON, **HC_COUNTY_DISTRICT_CANON}
DISTRICT_SHORT = {
    "東區": "東", "北區": "北", "香山區": "香山",
    "竹北市": "竹北", "竹東鎮": "竹東", "新埔鎮": "新埔", "關西鎮": "關西",
    "湖口鄉": "湖口", "新豐鄉": "新豐", "芎林鄉": "芎林", "橫山鄉": "橫山",
    "北埔鄉": "北埔", "寶山鄉": "寶山", "峨眉鄉": "峨眉", "尖石鄉": "尖石",
    "五峰鄉": "五峰",
}
...
PHONE_RE = re.compile(
    ...
    r"|\(?0?3\)?[-\s]?\d{3,4}[-\s]?\d{4}"  # Hsinchu / Taoyuan landline
...
ADDR_RE = re.compile(
    r"((?:新竹[市縣])?[\u4e00-\u9fff]{0,8}(?:路|街|道|大道|巷|弄|段)[^\s;；|]{0,30}\d+[-\d]*號?(?:之\d+)?(?:\d+樓)?)"
)
...
def pick_district(area: str, city: str) -> str:
    area_n = nfkc(area)
    multi_fallback = f"{city}多區" if city in ("新竹市", "新竹縣") else "新竹市多區"
    keys = CITY_DISTRICT_KEYS.get(city, [])
    # Prefer longer keys (香山 over …; 竹東 before any short clash)
    found = [k for k in keys if k in area_n]
    if found:
        found.sort(key=len, reverse=True)
        return DISTRICT_CANON[found[0]]
    # Full canon token after split
    for tok in re.split(r"[・·/／、,，\s（）()→\-]+", area_n):
        tok = tok.strip()
        if not tok:
            continue
        if city == "新竹市" and tok in HC_CITY_DISTRICT_CANON:
            return HC_CITY_DISTRICT_CANON[tok]
        if city == "新竹縣" and tok in HC_COUNTY_DISTRICT_CANON:
            return HC_COUNTY_DISTRICT_CANON[tok]
    if re.search(r"多店|多門市|多區|multi|citywide|跨區|跨市縣", area_n, re.I):
        return multi_fallback
    if area_n.strip() in {city, "新竹"}:
        return multi_fallback
    return multi_fallback
...
def make_id(num: int, existing_ids: set[str]) -> str:
    for cand in (f"{ID_PREFIX}-{num}", f"hc-scout-corpus-{num}", f"hc-lead-{num}-{TODAY.replace('-', '')}"):
        if cand not in existing_ids:
            return cand
    return f"hc-lead-{num}-{TODAY.replace('-', '')}-x"
...
def merge_into(root: Path, md: str) -> dict:
    ...
    report = {
        ...
        "policy": (
            ...
            "hosts. city=新竹市 OR 新竹縣 (separate); IDs: hc-scout-lead-{n}. Phones include 03-."
        ),
        "checkedAt": TODAY,
        "corpus": str(CORPUS),
        "root": str(root),
...
def main() -> None:
    ...
    for root in roots:
        ...
        out = root / "hsinchu-leads-merge-report.json"
